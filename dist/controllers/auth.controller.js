...
const login = async (req, res, next) => {
    try {
        ...
        // Verify password
        const isPasswordValid = await crypto_helper_1.default.compareAsync(password, user.password);
        if (!isPasswordValid) {
            return (0, http_status_1.sendBadRequestResponse)(res, 'Invalid email or password.');
        }
        // Generate JWT token
...
const forgotPassword = async (req, res, next) => {
    ...
    try {
        ...
        // Hash new password
        const hashedNewPassword = await crypto_helper_1.default.hashAsync(newPassword);
        // Update user password
        const isUserUpdated = await user_service_1.default.updateUser(user.id, { password: hashedNewPassword }, transaction);
        if (!isUserUpdated) {
            await transaction.rollback();
...
const changePassword = async (req, res, next) => {
    ...
    try {
        ...
        const isCurrentPasswordValid = await crypto_helper_1.default.compareAsync(currentPassword, user.password);
